...
class importOfertes(grok.View):
    ...
    def createOffers(self, hasHeaders, fitxer, marketUID):
        ...
        for count, row in enumerate(csv_file):
            ...
            notValidDegrees = self.checkNotValidDegrees(row[4].decode("utf-8").split(","))
            if len(notValidDegrees) == 0:
                teacher = getLdapExactUserData(row[6].decode("utf-8"))
                if teacher:
                    data = {
                        ...
                        'degree': row[4].decode("utf-8").split(","),
                        'keys': row[5].decode("utf-8").split(","),
                        ...
                        'codirector': row[7].decode("utf-8"),
                        'num_students': int(row[8].decode("utf-8")),
                        'workload': row[9].decode("utf-8"),
                        'targets': row[10].decode("utf-8"),
                        'features': row[11].decode("utf-8"),
                        'requirements': row[12].decode("utf-8"),
                        'lang': row[13].decode("utf-8").split(","),
                        'modality': row[14].decode("utf-8"),
                        'co_manager': row[15].decode("utf-8"),
                        'company': row[16].decode("utf-8"),
                        'company_contact': row[17].decode("utf-8"),
                        'company_email': row[18].decode("utf-8"),
                        'grant': bool(row[19].decode("utf-8") == "True"),
                        'confidential': bool(row[20].decode("utf-8") == "True"),
                        'environmental_theme': bool(row[21].decode("utf-8") == "True"),
                        'scope_cooperation': bool(row[22].decode("utf-8") == "True"),
                    }
                    ...
                    msg = row[0].decode("utf-8") + " - Teacher (" + row[6].decode("utf-8") + ") not exist."
