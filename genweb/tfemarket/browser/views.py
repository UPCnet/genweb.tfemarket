...
class getInfoCreateApplication(grok.View):
    ...
    def getIdPrisma(self, cn):
        ...
        if user['ok']:
            ...
            data = []
            for key in keys:
                for ind, i in enumerate(user['content'][0][key]):
                    try:
                        data[ind].update({key: i})
                    except IndexError:
                        data.append({key: i})
            for item in data:
                if item['typology'] == 'EST':
                    idPrisma = item['idorigen']
                    return idPrisma
        return ''
    ...
    def render(self):
        ...
        id_prisma = self.getIdPrisma(current.id)
        if id_prisma:
            data.update({'idPrisma': id_prisma})
            ...
            if res_data.ok:
                ...
                self.context.plone_utils.addPortalMessage(_(u"PRISMA id not found."), 'error')
