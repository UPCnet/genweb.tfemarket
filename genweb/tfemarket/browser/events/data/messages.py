# -*- coding: utf-8 -*-

M1 = {
'ca' : """\
Benvolgut / Benvolguda,

{student} que cursa {degree}, ha sol·licitat el treball final d’estudis {num} - {title} que tens ofertat.

Pots accedir a la sol·licitud des d’aquest enllaç {linkApplication} per tal de acceptar-la o rebutjar-la o bé accedir a totes les sol·licituds d’aquest treball {linkOffer}.

També pots accedir al Mercat de TFE {linkMarket} de l’escola directament.

Atentament,

{firm}
""",

'en' : '',

'es' : '',
}

M2 = {
'ca' : """\
Benvolgut / Benvolguda,

Et comuniquem que la teva sol·licitud per al següent treball de final d’estudis {num} - {title} ha estat acceptada. Recorda que per tal de finalitzar el procés d’assignació, has de confirmar-la o renunciar-hi des del següent enllaç {linkApplication}.

Un cop la confirmis, les teves dades arribaran a PRISMA per tal de realitzar la inscripció del treball.

Atentament,

{firm}
""",

'en' : '',

'es' : '',
}

M3 = {
'ca' : """\
Benvolgut / Benvolguda,

La teva sol·licitud per a l’oferta {num} - {title} ha estat rebutjada. Pot posar-te en contacte amb el professor per a qualsevol informació addicional.

Atentament,

{firm}
""",

'en' : '',

'es' : '',
}

M4 = {
'ca' : """\
Benvolgut / Benvolguda,

{student} ha confirmat la sol·licitud del treball final d’estudis {num} - {title}.

Pots accedir a la sol·licitud des d’aquest enllaç {linkApplication}, o bé accedir a totes les sol·licituds d’aquest treball {linkOffer}.

Recorda modificar l’estat de l’oferta i de les possibles sol·licituds que hi hagi, si s’ha assolit el número d’estudiants necessaris. També pots accedir al Mercat de TFE {linkMarket} de l’escola directament, per gestionar-lo.

Les dades es traspasaran a PRISMA per tal de realitzar l’inscripció del treball.

Atentament,

{firm}
""",

'en' : '',

'es' : '',
}

M5 = {
'ca' : """\
Benvolgut / Benvolguda,

{student} que va realitzar una sol·licitud a l’oferta {num} - {title} ha renunciat a la mateixa.

Atentament,

{firm}
""",

'en' : '',

'es' : '',
}

M6 = {
'ca' : """\
Benvolgut / Benvolguda,

{student} ha anul·lat la seva sol·licitud a l’oferta {num} - {title}.

Atentament,

{firm}
""",

'en' : '',

'es' : '',
}
